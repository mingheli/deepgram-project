        ...
        <>
            {audioList.length > 0 && audioList.map((file, index) => (
                <div className={`data-row ${currentFile?.id === file?.id ? "selected" : ""}`} key={index}>
                    ...
                    <div className="data-cell"><div className={`transcribe-action ${file.transcript ? "active" : ""}`} onClick={() => file.transcript ? handleTranscribe(file) : refetchData(file)}>{`${file.transcript ? "TRANSCRIBE" : ""}`}</div></div>
